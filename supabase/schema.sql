...
create table public.asignaciones_contenido (
  id uuid primary key default gen_random_uuid(),
  estudiante_id uuid not null references public.usuarios (id) on delete cascade,
...
create unique index asignaciones_contenido_test_uidx
  on public.asignaciones_contenido (estudiante_id, test_id)
  where test_id is not null;

create unique index asignaciones_contenido_leccion_uidx
  on public.asignaciones_contenido (estudiante_id, leccion_id)
  where leccion_id is not null;

create index asignaciones_contenido_test_idx
  on public.asignaciones_contenido (test_id)
  where test_id is not null;

create index asignaciones_contenido_leccion_idx
...
-- asignado va al final para poder usar CREATE OR REPLACE en migraciones.
