...
export default function ContentDashboard({
  ...
  async function borrar(contenido) {
    if (!window.confirm(`¿Eliminar “${contenido.titulo}”?`)) return;
    ...
    <>
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-slate-500">{texto.descripcion}</p>
        </div>
        {esAdmin && (
          <button
          ...
          {contenidos.map((contenido) => (
            ...
            >
              {tipoPagina === "vocabulario" && (
                <span className="mb-3 w-fit rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-medium capitalize text-indigo-700">
                  {contenido.tipo}
                </span>
              )}
